package io.github.treesitter.jtreesitter.quo;

import java.lang.foreign.*;

public final class TreeSitterQuo {
    ...
    private static final TreeSitterQuo INSTANCE = new TreeSitterQuo();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterQuo() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_quo");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-quo");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-quo");
